...
| RC + telemetry link | SIYI HM30 (shared with Quiver), ExpressLRS + SiK 915 or RFD900x, Radiolink AT9S Pro | **RC: Radiolink AT9S Pro TX + R9DS RX (SBUS).** Telemetry: STORK-borrowed Holybro SiK V3 for first flight, long-term TBD | RC side closed June 18: the AT9S Pro is the transmitter the pilots will use, on SBUS into the 6C RC-IN port. PT1 is VLOS ≤5 km. SBUS is unidirectional, so RC and ground telemetry stay separate (no CRSF MAVLink passthrough), and the SiK V3 carries MAVLink on TELEM1 as a range-limited first-flight unit. Upgrade to an RFD900x-class radio before the Phase 3 extended-range runs |
