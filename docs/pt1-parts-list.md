...
**Reference:** SPH-E-001 (electrical master, Rev 0.16) §4.16 cost baseline, §4.6 harness connection table
...
|---|---|---|---|---|---|---|---|
| 1 | [Flight controller](https://www.aykuthavacilik.com/urun/holybro-pixhawk-6x-standart-set-v2a-m9n-gps-pm02d) | **Pixhawk 6X Standard Set v2A** (FC module + Standard Baseboard v2A + PM02D HV power module + cable set + **M9N GPS included**) | 1 | Holybro EU (~$321). Aykut (TR) lists 38,220 TL (~$955) and is **out of stock** | $321 | Medium | **D4 resolved June 11: 6X selected.** Its IO PWM pins drive the wing flaperons directly, so the wing CAN-PWM node is dropped and shelf space is saved. TR route is ~3× Holybro direct and on backorder, order from Holybro EU unless Aykut restocks this week. The M9N (serial, not DroneCAN) becomes a bench and backup GPS |
| 2 | [CAN-to-PWM](https://www.aykuthavacilik.com/urun/ap-periph-can-node-can-l4-pwm) | **Matek CAN-L4-PWM** (AP_Periph, 9 PWM, JST-GH CAN, 3.5g) | 2 | Aykut Havacılık (TR), 2,060 TL (~$50) | $50 | Low (TR stock) | 1 install (tail, node 12, ruddervators) + 1 spare. The wing unit was dropped June 11 when the 6X was selected, its IO PWM drives the flaperons. Gates WP-E04 bench validation. Alternate at the same shop: CAN-L431 (5 PWM, 3 UART, I2C, dual parallel GH-4P CAN, 2,500 TL) if the L4-PWM sells out |
| 3 | [Voltage regulators](https://www.aykuthavacilik.com/urun/power-module-12s-w-3xbec) | **Matek PM12S-3** (9–55V in w/ TVS. Per unit: Vx selectable 5.25/6/8V at 15A cont / 25A peak, fixed 5.2V/4A, fixed 12V/4A, 1K:20K divider, 53g) | 2 | Aykut Havacılık (TR), 6,900 TL (~$170) | $170 | Low (TR stock) | Need at least two. Tail unit: Vx (5.25V PT1, 8V final) for ruddervators, 5V BEC for tail adapter + Here4. Forward unit: Vx for the flaperon rail, its 12V is a second contactor coil option. Vx switches to 8V for the final servos with no hardware change. **Price flag:** TR price is roughly 4× Matek direct (~$40/unit), about $260 extra across the pair, accepted because Turkish customs can be a hassle and the build is close. If the order slips to an EU batch anyway, buy these there |
| 4 | Tail GPS | **CubePilot Here4** (secondary GPS, node 11) | 1 | Aeroboticshop (EU) | $300 | Medium | Second unit pending the nose GPS decision (D1 below). If the team picks a matched pair, change qty to 2 |
...
| 6 | [Battery](https://www.robotsepeti.com/profuse-12s-444v-lipo-li-po-lityum-polimer-batarya-16000ah-60c?srsltid=AfmBOoqMvOkES6uTW2JkmBUoYShc7-rnFuaT4GsA0h9E_BRwsa5nnOG9LOo) | **ProFuse Super Nano 12S 16,000 mAh 60C** (44.4V, 3,822g, 186×76×160mm, XT150 socket) | 1 | robotsepeti.com, 41,322 TL incl. VAT (~$1,030), in stock | ~$1,030 | Low (TR stock) | D5 resolved: single pack. Send dimensions to Alperen, the 160mm height drives the forward bay. 60C clears the ≥15C floor by 4× |
...
|---|---|---|---|---|---|---|
| 7 | PT1 test servos | Any 5V digital metal gear, standard size, ≥6 kg·cm | 6 | Local TR hobby shop | $20 | 4 install (2 flaperon + 2 ruddervator) + 2 spares. Test only, final HV servos wait on WP-E06 |
...
| 10 | Battery connectors | XT150 male ×3 (mates the ProFuse pack socket) + XT150 female ×2 (charge lead and bench adapter), plus one optional AS150U adapter pigtail (only if D2 lands software-only) | 1 lot | Amass / TR hobby | $20 | The pack's XT150 is the ground safing disconnect. XT150 is not anti-spark: with an inline hardware kill, connect with the kill open and there is no mating spark |
...
### Battery interface build (how the PM02D pairs to the battery)

No Y-cable. The FCHUB-12S battery pads are the split point:

1. Battery XT150 male → 8 AWG red/black, ~0.4 m → solder to the FCHUB B+ / B− pads. The HV kill (D2) goes inline in this run.
2. At the same pads, solder a 16 AWG pigtail ending in an XT60 female → PM02D HV XT60 input. The PM02D sees pack voltage at the same electrical node a Y-splitter would give it.
3. PM02D XT60 output: unused, insulate it. Its 6-pin CLIK-Mate cable goes to Pixhawk POWER1 (5.2V + I2C).

Fallback if two wire sets crowd the pads: solder both the 8 AWG main and the 16 AWG branch into the XT150 male solder cups together (8 AWG + 16 AWG fit a 6mm cup), which moves the split into the connector barrel. Electrically identical. Pick one approach, not both.

## Priority 3 — blocked on a decision (do not order yet)

| ID | Decision | Options on the table | Owner | Needed by |
|---|---|---|---|---|
| D1 | Nose (primary) GPS, OQ-12 | (a) second Here4: matched RTK pair, enables moving baseline GPS yaw, one SKU. (b) F9P module or salvaged F9P with the antennas on hand: cheaper, compass yaw only | Team call | Before the main order ships |
| D2 | HV kill implementation, OQ-05 | Criterion set June 11: **must be switchable from the RC link.** (a) EV200AAANA contactor, ~€149, ~450g, coil driven straight from the FCHUB V2 12VSW pad via FC PINIO, no extra driver hardware (RC + GCS kill). (b) MOSFET anti-spark switch (200A cont / 400A peak class, ~100–150g, ~$80): light but fails short, no flight heritage, enable line needs an interface. (c) Defer hardware kill: ArduPilot motor emergency stop on an RC switch + XT150 unplug for ground safing, recorded §9.3 deviation. The ProFuse pack's plain XT150 (no anti-spark) favors (a) or (b): connect with the kill open and there is no mating spark. Full analysis in SPH-E-001 §4.3 | Erick + Alperen | Next call |
...
| D4 | FC selection | Resolved June 11: **Pixhawk 6X** (line 1), chosen so the IO PWM pins drive the wing servos and the wing CAN-PWM node is saved. Remaining sourcing call: Holybro EU now vs wait for Aykut restock at ~3× the price | Alperen (sourcing) | Order day |
| D5 | Battery format | Resolved June 11: single ProFuse 12S pack, in stock at robotsepeti (line 6) | — | Closed |
...
| Block | Est. |
|---|---|
| Priority 1 (6X set EU, single Here4, ProFuse pack, 2× PM12S-3, 2× CAN-L4-PWM, 2× FCHUB-12S V2, US-D1 on hand) | ~$2,150 |
| Priority 2 | ~$390 |
| Decision items (D1 second Here4 + D2 contactor route, if both chosen) | ~$460 |
| If the US-D1 does not turn up in Turkey | +$280 |
| **PT1 order cycle total** | **~$2,540–3,280** |
